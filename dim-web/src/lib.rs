...
pub async fn start_webserver(
    ...
    let web_fut = axum::serve(listener, router);

    tokio::select! {
        res = web_fut => {
            if let Err(err) = res {
                tracing::error!(%err, "webserver exited with error");
            }
        },
        _ = shutdown_fut => {
            // graceful stop; axum::serve doesn't cancel itself automatically,
            // so we just return and drop the listener/service.
        }
    }
}
